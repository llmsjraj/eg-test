...
import { signInSchema } from '../validation/signupValidation';
...
import { SignInFormData } from '../types/formModels';
  ...
  const onSubmit = async (data: SignInFormData) => {
    try {
      ...
      if (error.response?.data?.errors && error.response.data.errors.length > 0) {
        ...
        setError(error.response?.data?.message || 'Error occurred during sign in');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Container maxWidth="sm">
      <Paper style={{ padding: '20px', marginTop: '20px' }}>
        <Typography variant="h5" style={{ marginBottom: '20px' }}>Sign In</Typography>
        ...
        <Grid item xs={12} style={{ marginTop: '20px', textAlign: 'center' }}>
          <Typography variant="body2">
            ...
            <Link href="#" onClick={navigateToSignUp} style={{ cursor: 'pointer' }}>
              Sign up
            </Link>
          </Typography>
        </Grid>
